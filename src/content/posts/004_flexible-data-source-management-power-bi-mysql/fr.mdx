...
Dans le cadre d’un projet pour l’un de nos clients, sur lequel je travaille actuellement (au moment de la rédaction de cet article), nous avons été confrontés à un défi intéressant sur la création d’un tableau de bord Power BI connecté à une base de données MySQL contenant des millions de lignes. Cette expérience nous a permis de développer une solution élégante pour gérer différentes sources de données en fonction de l’environnement de travail. Dans cet article, je vais vous présenter les différentes étapes de construction de cette solution, en abordant les problématiques rencontrées et les solutions mises en place.
...
```powerquery ins={"Ajouter les étapes de transformation spécifiques aux données CSV ici":14-17} del={"Supprimer":28} ins={"Remplacer":29}
let
    GetData = (NumberColumnsCSV as number, MySQLQuery as text) =>
        let
            CsvSource = Csv.Document(
...
Au final, nous avons trouvé une solution simple mais puissante pour contourner les défis de connexion et de performance. En quelques étapes, nous avons transformé un problème complexe en une approche flexible qui nous permet de travailler efficacement, que ce soit en local ou en production. Cette approche peut facilement être adaptée et étendue pour répondre à d'autres besoins par exemple ajout de nouveaux environnements (test, pré-production) ou support d'autres sources de données (PostgreSQL, Oracle, etc.).
